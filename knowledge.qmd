...
A practical inquiry can often be represented by the kind of decision table that we use in decision theory courses.[^knowledge-1] We take something as given in that inquiry iff it is encoded in the right way in the decision table. The primary way that we encode a proposition *p* into a decision table is to set up the table so that *p* is true in every column. If we use a table where *p* is encoded in this way, and *p* is not known, we are making a mistake. And in particular, we are making an epistemic mistake.

[^knowledge-1]: This section is based on §1.1 of my [-@Weatherson2012].
